...
// Simple 2-col form row (label left, blank right)
...
function makeLkpd01a(d) {
  ...
  els.push(bannerTable(`${s(d.judulLampiran || "LKPD-01a")} — ${s(d.subjudul)}`, C.navyDark, C.white, 24, `Metode: ${s(d.metode)}   |   ${s(d.pertemuan)}`));
  els.push(pEmpty(60));
  ...
  els.push(new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [
        formRow("Kelas:").children[0],
        formRow("Tanggal:").children[0],
      ]}),
    ],
  }));
  els.push(pEmpty(80));
  ...
  // Petunjuk
  els.push(bannerTable("PETUNJUK KERJA", C.navyMid, C.white, 20));
  ...
  // Tabel Analisis
  els.push(bannerTable("TABEL ANALISIS", C.navyMid, C.white, 20));
  ...
  if (gw.instruksi) {
    els.push(bannerTable("PENILAIAN ANTAR KELOMPOK", C.navyMid, C.white, 20));
...
function makeLkpd01b(d) {
  ...
  els.push(bannerTable(`${s(d.judulLampiran || "LKPD-01b")} — ${s(d.subjudul)}`, C.navyDark, C.white, 24, s(d.deskripsiPertemuan)));
  els.push(pEmpty(60));
  ...
  els.push(new Table({
    ...
    rows: [
      new TableRow({ children: [formRow("Nama / Kelompok:").children[0], formRow("Kelas:").children[0]] }),
      new TableRow({ children: [formRow("Tanggal:").children[0], dCell(p(s(hf.tujuan), { size: 18, color: "555555" }), { w: CONTENT_W / 2 })] }),
    ],
  }));
  els.push(pEmpty(80));
  ...
  // Bagian A
  const bA = d.bagianA || {};
  els.push(bannerTable(s(bA.judul || "BAGIAN A"), C.navyMid));
  ...
  // Bagian B
  const bB = d.bagianB || {};
  els.push(bannerTable(s(bB.judul || "BAGIAN B"), C.navyMid));
  ...
  // Bagian C
  const bC = d.bagianC || {};
  els.push(bannerTable(s(bC.judul || "BAGIAN C"), C.navyMid));
  ...
  // Bagian D
  const bD = d.bagianD || {};
  els.push(bannerTable(s(bD.judul || "BAGIAN D — REFLEKSI"), C.navyMid));
  els.push(new Table({
    width: { size: CONTENT_W, type: WidthType.DXA },
    columnWidths: [CONTENT_W],
    rows: [new TableRow({ children: [dCell([
...
function makeAsesmenFormatif(d) {
  ...
  els.push(bannerTable(`${s(d.judulLampiran || "Instrumen Asesmen Formatif")}`, C.navyDark, C.white, 24, s(d.subjudul)));
  els.push(pEmpty(60));
  ...
  const bA = d.bagianA || {};
  els.push(bannerTable(s(bA.judul || "A. PERTANYAAN LISAN"), C.navyMid));
  ...
  // Bagian B — Soal Kuis
  const bB = d.bagianB || {};
  els.push(bannerTable(s(bB.judul || "B. SOAL KUIS TULIS"), C.navyMid));
...
function makeAsesmenSumatif(d) {
  ...
  els.push(bannerTable(s(d.judulLampiran || "Instrumen Asesmen Sumatif"), C.navyDark, C.white, 24, s(d.subjudul)));
  els.push(pEmpty(60));
  ...
  els.push(bannerTable("RUBRIK PENILAIAN", C.navyMid));
  els.push(new Table({
    width: { size: CONTENT_W, type: WidthType.DXA },
    columnWidths: colR,
    rows: [
  ...
  if (ts.kolomSummary) {
    ...
    els.push(new Table({
      ...
      columnWidths: sumW,
      rows: [
        new TableRow({ children: sumCols.map((h, i) => hCell(h, { w: sumW[i], bg: C.navyLight, size: 18 })) }),
        new TableRow({ children: [
  ...
  if (d.catatanGuru) {
    els.push(pEmpty(80));
    els.push(bannerTable("CATATAN GURU", C.navyLight, C.textDark, 20));
    els.push(new Table({
      width: { size: CONTENT_W, type: WidthType.DXA },
      columnWidths: [CONTENT_W],
...
function makeRekapKelas(d) {
  ...
  els.push(bannerTable(s(d.judulLampiran || "Rekap Kelas"), C.navyDark, C.white, 24, s(d.subjudul)));
  els.push(pEmpty(60));
  ...
  els.push(new Table({
    ...
    rows: [
      ...
      // Rata-rata row
      new TableRow({ children: [
        dCell(p("", { size: 18 }), { w: noW, bg: C.navyLight }),
        dCell(p("Rata-rata Kelas", { bold: true, size: 18 }), { w: namaW, bg: C.navyLight }),
        ...aspekArr.map((_, ai) => answerCell({ w: allWidths[2 + ai] })),
        ...kolTambahan.map((_, ki) => answerCell({ w: tambahW })),
      ]}),
    ],
...
function makeMedia(d) {
  ...
  els.push(bannerTable(s(d.judulLampiran || "Media Pembelajaran"), C.navyDark, C.white, 24, s(d.subjudul)));
  els.push(pEmpty(60));
  ...
  // Bagian A — Slide
  const bA = d.bagianA || {};
  els.push(bannerTable(s(bA.judul || "A. RINCIAN SLIDE"), C.navyMid));
  ...
  // Bagian B — Referensi
  const bB = d.bagianB || {};
  els.push(bannerTable(s(bB.judul || "B. REFERENSI VIDEO & SUMBER DIGITAL"), C.navyMid));
...
function makeLembarRefleksi(d) {
  ...
  els.push(bannerTable(s(d.judulLampiran || "Lembar Refleksi"), C.navyDark, C.white, 24, `Teknik: ${s(d.teknik || "3-2-1")}   |   ${s(d.keterangan)}`));
  els.push(pEmpty(60));

  const pertemuanArr = Array.isArray(d.pertemuan) ? d.pertemuan : [];
  pertemuanArr.forEach((prt, idx) => {
    if (idx > 0) els.push(pEmpty(120));
    els.push(bannerTable(s(prt.labelPertemuan || `Pertemuan ${idx + 1}`), C.navyMid));
    ...
    els.push(new Table({
      ...
      columnWidths: [CONTENT_W / 2, CONTENT_W / 2],
      rows: [
        new TableRow({ children: [formRow("Nama:").children[0], formRow("Kelas:").children[0]] }),
        new TableRow({ children: [
          formRow("Tanggal:").children[0],
          dCell(p(`Topik: ${s(hf.topik)}`, { size: 18, color: "555555" }), { w: CONTENT_W / 2 }),
        ]}),
      ],
    }));
    els.push(pEmpty(60));
...
function makePengayaan(d) {
  ...
  els.push(bannerTable(s(d.judulLampiran || "Bahan Pengayaan"), C.navyDark, C.white, 24, s(d.subjudul)));
  els.push(pEmpty(60));
  ...
  materiArr.forEach((mat, i) => {
    els.push(bannerTable(s(mat.judul || `Materi ${i + 1}`), C.navyMid));
  ...
  if (sc.judul) {
    els.push(bannerTable(s(sc.judul), C.navyMid));
  ...
  if (tugas.instruksi) {
    els.push(bannerTable("TUGAS PENGAYAAN", C.navyMid));
...
function makeRemediasi(d) {
  ...
  els.push(bannerTable(s(d.judulLampiran || "Bahan Remediasi"), C.navyDark, C.white, 24, s(d.subjudul)));
  els.push(pEmpty(60));
  ...
  materiArr.forEach((mat, i) => {
    els.push(bannerTable(`${mat.nomor || i + 1}. ${s(mat.judul)}`, C.navyMid));
  ...
  els.push(bannerTable("LATIHAN SOAL", C.navyMid));
  els.push(pEmpty(40));
  els.push(new Table({
    width: { size: CONTENT_W, type: WidthType.DXA },
    columnWidths: [CONTENT_W / 2, CONTENT_W / 2],
    rows: [
      new TableRow({ children: [formRow("Nama:").children[0], formRow("Kelas:").children[0]] }),
      new TableRow({ children: [formRow("Tanggal:").children[0], answerCell({ w: CONTENT_W / 2 })] }),
    ],
  }));
  els.push(pEmpty(60));
