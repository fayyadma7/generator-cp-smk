...
function makeBagian1() {
  ...
  const mainBanner = new Table({
    ...
    rows: [new TableRow({ children: [new TableCell({
      ...
      children: [
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 60 }, children: [new TextRun({ text: "FORMAT CAPAIAN PEMBELAJARAN", bold: true, size: 36, color: C.white, font: "Arial" })] }),
  ...
  const tujuanTable = new Table({
    ...
    columnWidths: [1600, CONTENT_W - 1600],
    rows: [
      new TableRow({ children: [hCell("TUJUAN", { bg: C.navyMid }), hCell("Uraian Tujuan Sekolah", { bg: C.navyMid })] }),
      new TableRow({ children: [
        dCell(p("Tujuan Umum", { bold: true, size: 20, align: AlignmentType.CENTER }), { bg: C.navyLight }),
        dCell(tujuanUmumList.map((t, i) => p(`${i + 1}. ${t}`, { size: 20, spBefore: 20, spAfter: 20 })), { bg: C.white }),
      ]}),
      new TableRow({ children: [
        dCell(p("Tujuan Khusus", { bold: true, size: 20, align: AlignmentType.CENTER }), { bg: C.navyLight }),
...
function makeBagian2() {
  ...
  const tabel = new Table({
    ...
    columnWidths: W,
    rows: [
      new TableRow({ children: [hCell("No", { w: W[0] }), hCell("Dimensi Profil Lulusan", { w: W[1] }), hCell("Deskripsi", {})] }),
      ...dimensi.map((d, i) => new TableRow({ children: [
        dCell(p(d.no, { bold: true, size: 22, align: AlignmentType.CENTER }), { w: W[0], bg: i % 2 === 0 ? C.navyLight : C.gray, vAlign: VerticalAlign.CENTER }),
        dCell(d.nama.split("\n").map(l => p(l, { bold: true, size: 20, spBefore: 20, spAfter: 20 })), { w: W[1], bg: i % 2 === 0 ? C.white : C.gray }),
...
function makeBagian3() {
  ...
  const tabel = new Table({
    ...
    columnWidths: W,
    rows: [
      new TableRow({ children: [hCell("Bag.", { w: W[0] }), hCell("Komponen", { w: W[1] }), hCell("Petunjuk Pengisian", {})] }),
      ...petunjuk.map((d, i) => new TableRow({ children: [
        dCell(p(d.bag, { bold: true, size: 22, align: AlignmentType.CENTER, color: C.navyDark }), { w: W[0], bg: i % 2 === 0 ? C.navyLight : C.grayDark, vAlign: VerticalAlign.CENTER }),
...
function makeBagian4(data, aiData) {
  ...
  const elemenList = Array.isArray(data.elemenList) && data.elemenList.length > 0
    ? data.elemenList
  ...
  const elemenTableRows = [
    new TableRow({ children: [
      hCell("Elemen", { w: WB2[0], bg: C.navyMid }),
      hCell("Capaian Pembelajaran", { bg: C.navyMid }),
    ]}),
    ...(elemenList.length > 0
      ? elemenList.filter(el => el.nama && el.nama.trim()).map((el, i) =>
          new TableRow({ children: [
            dCell(p(el.nama, { bold: true, size: 20 }), { w: WB2[0], bg: i % 2 === 0 ? C.white : C.gray }),
  ...
  const mainTable = new Table({
    ...
    rows: [
      // ── A. IDENTITAS ──
      secRow("A. IDENTITAS MATA PELAJARAN"),
      ...
      secRow("B. CAPAIAN PEMBELAJARAN"),
      subSecRow("B.1  Capaian Pembelajaran Umum (dari Kepmendikbudristek)"),
      b1Row,
      subSecRow("B.2  Capaian Pembelajaran Per Elemen"),
      b2EmbedRow,

      // ── C. ANALISIS & KONTEKSTUALISASI ──
      secRow("C. ANALISIS & KONTEKSTUALISASI CP"),
      ...
      // ── D. 8 DIMENSI ──
      secRow("D. KETERKAITAN DENGAN 8 DIMENSI PROFIL LULUSAN"),
      new TableRow({ children: [
      ...
      // ── E. DEEP LEARNING ──
      secRow("E. IMPLEMENTASI DEEP LEARNING"),
      ...
      // ── F. STRATEGI ──
      secRow("F. REKOMENDASI STRATEGI PEMBELAJARAN"),
      ...
      secRow("G. CATATAN PENGEMBANGAN & VALIDASI"),
  ...
  return [
    bannerTable("BAGIAN IV \u2014 FORMAT ISIAN GURU", C.navyDark, C.white, 26,
...
function makeBagian5() {
  const steps = [
    { no: "1", icon: "📋", label: "Format CP",             sub: "Dokumen ini" },
  ...
  // Buat kolom: [kotak1][panah][kotak2][panah][kotak3][panah][kotak4][panah][kotak5]
  // Setiap kotak = ~1600, panah = ~400
  const BOX = 1700;
  const ARR = 350;
  const totalW = BOX * 5 + ARR * 4;
  const colWidths = [];
      ...
      new TableRow({
        children: steps.flatMap((s, i) => {
          const box = new TableCell({
            ...
            borders: allBorders(C.navyMid, 4),
            shading: { fill: i === 0 ? C.navyDark : i === 4 ? C.green : C.navyMid, type: ShadingType.CLEAR },
            ...
            children: [
              ...
              new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 0 }, children: [new TextRun({ text: s.sub, size: 16, color: C.navyLight, font: "Arial" })] }),
            ],
          });
          if (i < 4) {
  ...
  return [
    bannerTable("BAGIAN V \u2014 ALUR PENGGUNAAN DOKUMEN CP", C.navyMid, C.white, 22,
      "Gunakan Format CP ini sebagai titik awal penyusunan seluruh perangkat pembelajaran"),
    pEmpty(100),
    alurTable,
    pEmpty(80),
    catatanGuru,
