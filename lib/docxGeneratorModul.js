...
// ═══════════════════════════════════════════════════════════
const C = {
  // Classic Core -> Vibrant Blue
  navyDark:    "1E3A8A", // Blue 900
  navyMid:     "2563EB", // Blue 600
...
function p(text, opts = {}) {
  const {
    ...
    color = C.textDark, align = AlignmentType.JUSTIFIED,
...
function pMulti(runs, opts = {}) {
  const { align = AlignmentType.JUSTIFIED, spBefore = 40, spAfter = 40 } = opts;
  return new Paragraph({
    alignment: align,
    spacing: { before: spBefore, after: spAfter },
...
function parseMarkdownRuns(line, defaultOpts = {}) {
  ...
  // Hilangkan tanda kurung siku di awal & akhir jika ada (misal: [Langkah 1 — Siswa berdoa])
  // Pastikan itu adalah satu blok kurung siku tunggal (tidak mengandung ] di dalamnya)
  if (cleanLine.startsWith("[") && cleanLine.endsWith("]") && !cleanLine.slice(1, -1).includes("]")) {
...
function pMarkdown(text, opts = {}) {
  const { align = AlignmentType.JUSTIFIED, spBefore = 40, spAfter = 40, ...rest } = opts;
  const runs = parseMarkdownRuns(String(text || ""), rest);
  if (runs.length === 0) {
    return new Paragraph({
      alignment: align,
      spacing: { before: spBefore, after: spAfter },
      children: [new TextRun({ text: "", font: "Arial", ...rest })]
    });
  }
  return new Paragraph({
    alignment: align,
...
function valueCell(content, w) {
  ...
  if (Array.isArray(content)) {
    paras = content.map(l => pMarkdown(`• ${l}`, { size: 20, spBefore: 20, spAfter: 20 }));
  } else if (typeof content === 'object') {
    Object.entries(content).forEach(([k, v]) => {
      paras.push(pMulti([{ text: `${k}: `, bold: true, font: "Arial" }, ...parseMarkdownRuns(String(v), { size: 20, color: C.textDark })], { spBefore: 20, spAfter: 20 }));
    });
  } else {
    const lines = String(content).split("\n");
    paras = lines.map(l => pMarkdown(l, { size: 20, spBefore: 20, spAfter: 20 }));
  }
  ...
  skenarioData.forEach((sken, idx) => {
    ...
    fases.forEach(f => {
      ...
      if (Array.isArray(f.data?.kegiatan_guru)) {
         guruParas = f.data.kegiatan_guru.map((k,i) => pMarkdown(`${i+1}. ${k}`, { size: 18, spBefore: 20, spAfter: 20 }));
      } else {
         guruParas = [pMarkdown(String(f.data?.kegiatan_guru || "-"), { size: 18 })];
      }

      if (Array.isArray(f.data?.kegiatan_peserta_didik)) {
         siswaParas = f.data.kegiatan_peserta_didik.map((k,i) => pMarkdown(`${i+1}. ${k}`, { size: 18, spBefore: 20, spAfter: 20 }));
      } else {
         siswaParas = [pMarkdown(String(f.data?.kegiatan_peserta_didik || "-"), { size: 18 })];
      }
...
function makeRubrikRows(aiData) {
  ...
  aspekData.forEach(a => {
    rows.push(new TableRow({ children: [
      dCell(pMarkdown(a.nama, { bold: true })),
      dCell(pMarkdown(a.level_1?.deskriptor || "-")),
      dCell(pMarkdown(a.level_2?.deskriptor || "-")),
      dCell(pMarkdown(a.level_3?.deskriptor || "-")),
      dCell(pMarkdown(a.level_4?.deskriptor || "-")),
    ]}));
  });
