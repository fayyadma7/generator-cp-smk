...
export async function POST(request) {
  try {
    ...
    // Pakai aiClient: otomatis failover Gemini → NVIDIA → Groq
...
${jsonFormat}`;
    ...
    // Panggil AI dengan failover otomatis Gemini → NVIDIA → Groq
    const aiResult = await generate({ system: systemPrompt, user: finalUserPrompt, jsonMode: true });
    const result = parseAIResult(aiResult.text);
    console.log(`[generate-cp] Berhasil via ${aiResult.provider}`);
